...
export async function sendVerificationEmail(correo, otp, nombre = 'Usuario') {
  ...
  if (mailer && config.SMTP_FROM) {
    try {
      await mailer.sendMail({
        ...
        to: correo,
        subject: `NexoAdmin · Código de activación: ${otp}`,
        text: `Hola ${nombre},\n\nTu código de verificación para activar tu cuenta en NexoAdmin es: ${otp}\n\nEste código vence en 15 minutos.\nSi no solicitaste esta cuenta, puedes ignorar este mensaje.\n\nEquipo NexoAdmin`,
        html: `
          <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 560px; margin: 0 auto; padding: 32px 28px; background: #ffffff; border: 1px solid #ccdbfd; border-radius: 14px; color: #000000;">
            <div style="margin-bottom: 24px;">
              <span style="font-size: 11px; font-weight: 800; letter-spacing: 1.5px; color: #2563eb;">NEXOADMIN · ACTIVACIÓN DE CUENTA</span>
            ...
            <p style="font-size: 15px; color: #334155; line-height: 1.6;">Usa el siguiente código de seguridad de 6 dígitos para verificar tu cuenta y acceder a NexoAdmin:</p>
            ...
            <p style="font-size: 12px; color: #94a3b8; text-align: center; margin: 0;">NexoAdmin · Event Operating System</p>
          </div>
        `
      });
  ...
  console.info(`\n======================================================`);
  console.info(`[NEXOADMIN EMAIL SENDER · MODO LOCAL / DEV]`);
...
export async function sendRecoveryEmail(correo, otp) {
  ...
  if (mailer && config.SMTP_FROM) {
    await mailer.sendMail({
      from: config.SMTP_FROM,
      to: correo,
      subject: `NexoAdmin · Recupera tu acceso: ${otp}`,
  ...
  console.info(`\n[NEXOADMIN RECOVERY SENDER · MODO LOCAL] Para ${correo}: ${otp}\n`);
  return { sent: false, devCode: otp };
}
