...
export default function TicketModal({ saleId, onClose }) {
  ...
  const fallback = data ? <div className="ticket-flat"><span className="page-eyebrow">NEXOADMIN / EVENT PASS</span><h3>{data.evento.nombre}</h3><p>{dateLabel(data.evento.fecha)} · {data.evento.horario}</p><strong>{data.venta.nombreAsistente}</strong><img src={qr} alt="Código QR del boleto"/><code>{data.venta.ticketCode}</code></div> : null;
